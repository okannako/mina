(**
   ...
   It checks that the packages from the first opam.export file are installed in the second one (with the same versions).
  ...
  OpamPackage.Name.Map.iter
    (fun name overlay_file ->
      ...
      | Some overlay_current ->
         check_overlay_eq opam_export_filepath (OpamPackage.Name.to_string name) overlay_file overlay_current
    )
    overlays
...
let () =
  if Array.length Sys.argv <> 3 then
    let () = Format.printf "USAGE: check_opam_switch opam1.export opam2.export" in
    exit 1
  else
    let opam_export_filepath = Sys.argv.(1) in
    let current_filepath = Sys.argv.(2) in
    let opam_export = switch_export_of_path opam_export_filepath in
    let current = switch_export_of_path current_filepath in
    let () =
      check_pkgs_subset
        opam_export_filepath
